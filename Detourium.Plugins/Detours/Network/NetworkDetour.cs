...
namespace Detourium.Detours.Network
{
    ...
    public class InterceptResponse
    {
        ...
        public InterceptResponse(bool filter, byte[] buffer = null)
        {
            if (filter && (buffer == null || buffer.Length == 0))
                throw new NetworkDetourException("The buffer must be non-empty if the response is set to filter.");
    ...
    public class NetworkDetour : IDisposable
    {
        ...
        {
            Console.WriteLine("NetworkDetour: " + socket);
        ...
        /// </summary>
        public static byte[] GetStringToBytes(string value) => SoapHexBinary.Parse(value).Value;

        /// <summary>
        /// Convert bytes to a hexadecimal string.
        /// </summary>
        public static string GetBytesToString(byte[] value) => new SoapHexBinary(value).ToString();
    }
}
